...
<body>
  ...
  <main class='Main MainHome --full'>
    <div class='Main__landing'>
      <article class='landing__info'>
        <h2>Maintaining a healthy weight is important for health.</h2>
        <p>In addition to lowering the risk of heart disease, stroke, diabetes,
          and high blood pressure, it can also lower the risk of many different cancers.</p>
        <h1 class='bold'>Stay healthy with us!</h1>
      </article>

    <?php if (!isset($_SESSION['logged'])) {
      echo  "<a class='landing__login --button' href='login.php'>Login / Signup</a>";
     } else exit();
     ?>
    </div>
